...
class Emitter<EventsType extends Record<keyof EventsType, EventsType[keyof EventsType]>> {
  ...
  useEventListener = <K extends keyof EventsType, V extends EventsType[K]>(key: K, fallbackValue?: V) => {
    // @ts-ignore
    const [value, setValue] = createSignal<EventsType[K]>(this.getCache(key) || fallbackValue)
    const [listener, setListener] = createSignal<{ id: string; event: K; clearListener: () => void } | null>(null)

    onMount(() => {
      const eventListener = this.listen(key, setValue)
